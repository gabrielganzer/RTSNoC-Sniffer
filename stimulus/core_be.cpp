...
void core_be::prc_send() {
	...
	if (i_rst.read() == 1) {
		...
		case st_DISABLE_BODY:
			...
			if ((i_wait.read() == 0)&(n_flit < p_BEST_EFFORT + 1))
				st_STATE_SEND = st_BODY;
			//else if ((i_wait.read() == 0)&(n_flit == i))
			else if ((i_wait.read() == 0)&(n_flit == p_BEST_EFFORT + 1))
