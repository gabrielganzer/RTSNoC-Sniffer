...
SC_MODULE(output_fifo) {
	...
	sc_in<sc_uint<(p_DATA + 2 * p_SIZE_X + 2 * p_SIZE_Y + 7)> >			i_din;

	//Outputs
	sc_out<sc_uint<1> >													o_empty;
	sc_out<sc_uint<1> >													o_full;
	sc_out<sc_uint<1> >													o_almost_full;
	sc_out<sc_uint<(p_DATA + 2 * p_SIZE_X + 2 * p_SIZE_Y + 7)> >		o_dout;
	...
	SC_CTOR(output_fifo) {
		...
		rcl_unit = new addr_control_neg("read_control_logic");
		rcl_unit->clk(i_rclk);
		...
		rcnt_unit = new counter_neg("read_side_counter");
		...
		mem_unit = new output_memory("dual_port_ram");
		...
		sensitive << i_rclk.neg() << i_rst << w_empty;
